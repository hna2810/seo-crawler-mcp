import { PageData, SEOIssue } from "../crawler/types";
import { calculateJaccardSimilarity } from "../utils/text";
 ...
 */
export function hasRedirectOrCanonical(page: PageData): boolean {
  // 1. Soft 404 (redirect về trang chủ)
  if (page.isSoft404) return true;

  // 2. HTTP Redirect status (301, 302, 307, 308)
  if (page.statusCode >= 300 && page.statusCode < 400) return true;
  ...
  if (page.canonical && page.canonical.trim() !== "") {
    try {
      ...
      if (normCanonical !== normUrl) return true;
    }
  }
...
export function performSEOAudit(pages: Record<string, PageData>): SEOAuditReport {
  ...
  for (const page of pageList) {
    ...
    if (page.statusCode !== 200 || page.isSoft404) continue;
  ...
  // - Có nội dung >= 100 từ
  const validArticles = pageList.filter(
    p => p.statusCode === 200 &&
         !p.isSoft404 &&
         p.isArticle !== false &&
  ...
  const wordBuckets = new Map<number, typeof validArticles>();
  ...
  for (const [, bucketPages] of wordBuckets.entries()) {
    if (bucketPages.length < 2 || totalPairChecks >= MAX_PAIR_CHECKS) continue;

    for (let i = 0; i < bucketPages.length; i++) {
      for (let j = i + 1; j < bucketPages.length; j++) {
        if (totalPairChecks++ >= MAX_PAIR_CHECKS) break;
        ...
        if (canon1 === norm2 || canon2 === norm1) continue;
        if (p1.finalUrl && p1.finalUrl.replace(/\/+$/, "").toLowerCase() === norm2) continue;
        if (p2.finalUrl && p2.finalUrl.replace(/\/+$/, "").toLowerCase() === norm1) continue;

        const pairKey = p1.url < p2.url ? `${p1.url}|${p2.url}` : `${p2.url}|${p1.url}`;
        if (checkedPairs.has(pairKey)) continue;
        checkedPairs.add(pairKey);

        const sim = calculateJaccardSimilarity(p1.mainContentText, p2.mainContentText, 3);
        if (sim >= 0.85) {
