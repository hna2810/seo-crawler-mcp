import { PageData, SiteNode } from "../crawler/types";

export interface SiteStructureAnalysis {
  tree: SiteNode;
  asciiTree: string;
  depthDistribution: Record<number, number>;
  orphanPages: { url: string; title: string; depth: number }[];
  topLinkedPages: { url: string; title: string; inlinkCount: number }[];
  sectionsSummary: { section: string; pageCount: number }[];
}

export function buildSiteStructure(pages: Record<string, PageData>, rootUrl: string): SiteStructureAnalysis {
  let rootDomain = "";
  try {
  ...
  const orphanPages: { url: string; title: string; depth: number }[] = [];
  const topLinkedPages: { url: string; title: string; inlinkCount: number }[] = [];
...
function renderAsciiTree(node: SiteNode, prefix = "", isLast = true): string {
  let result = "";
