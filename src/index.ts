...
import { getSession, listSessions, getLatestSessionId } from "./crawler/session";
...
const AUDIT_SEO_ISSUES_TOOL: Tool = {
...
const ANALYZE_SITE_STRUCTURE_TOOL: Tool = {
...
const CLASSIFY_CONTENT_TOPICS_TOOL: Tool = {
...
const CLASSIFY_SINGLE_URL_OR_TEXT_TOOL: Tool = {
...
const EXPORT_CRAWL_REPORT_TOOL: Tool = {
    ...
    switch (name) {
      case "crawl_website": {
        ...
        const crawler = new WebsiteCrawler({
          ...
          sitemapUrl: (args as any)?.sitemapUrl,
          maxDepth: (args as any)?.maxDepth,
          maxPages: (args as any)?.maxPages,
          includePattern: (args as any)?.includePattern,
          excludePattern: (args as any)?.excludePattern,
          concurrency: (args as any)?.concurrency,
          delayMs: (args as any)?.delayMs
        });
      ...
      case "audit_seo_issues": {
        const sessionId = (args as any)?.sessionId || getLatestSessionId();
        ...
        const audit = performSEOAudit(session.pages);
        const filterSeverity = (args as any)?.filterSeverity || "all";
        const filterCategory = (args as any)?.filterCategory || "all";

        let displayedIssues: any[] = [];
        if (filterCategory === "all") {
          ...
          displayedIssues = (audit.issuesByCategory as any)[filterCategory] || [];
        }
      ...
      case "classify_single_url_or_text": {
        const inputUrl = (args as any)?.url;
        const title = (args as any)?.title;
        const h1 = (args as any)?.h1;
        const headings = (args as any)?.headings;
        const boldKeywords = (args as any)?.boldKeywords;
        const mainText = (args as any)?.mainText;
      ...
      case "export_crawl_report": {
        ...
        const mdReport = generateMarkdownReport(session, audit, structure, contentRatio, classifications);
        return {
