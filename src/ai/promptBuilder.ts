import { PageData, Outlink, Inlink, ClassificationResult } from "../crawler/types";
...
export interface AggregatedSEOStats {
  rootUrl: string;
  totalPages: number;
  totalArticles: number;
  totalNonArticles: number;
  depthDistribution: Record<number, number>;
  avgWordCountArticles: number;
  ...
  sample404Urls: string[];
  sampleDuplicateTitles: { title: string; count: number }[];
  
  // Structure & Hierarchy
  orphanPagesCount: number;
  sampleOrphanPages: string[];
  topSections: { section: string; count: number }[];
  
  // Content Ratio & Content Gap
  topicBreakdown: { topic: string; count: number; ratio: number }[];
  topTopicName: string;
  topTopicRatio: number;
  contentGaps: { topic: string; missingSubtopics: string[] }[];
  
  // Link Architecture & Link Gap
  avgInternalLinksPerArticle: number;
  articlesWithZeroInlinks: number;
  articlesWithOneInlink: number;
  isolatedArticlesSample: { url: string; title: string; inlinkCount: number }[];
  deadEndArticlesCount: number; // 0 internal outlinks
  sampleDeadEndArticles: { url: string; title: string }[];
  topLinkedArticles: { url: string; title: string; inlinkCount: number }[];
  
  // Anchor Text
  totalAnchorsEvaluated: number;
  genericAnchorsCount: number;
  sampleGenericAnchors: { anchor: string; count: number }[];
  sampleTopKeywordsAnchors: { anchor: string; count: number }[];
  
  // External Links
  externalDomainsLinked: { domain: string; count: number }[];
}

export function aggregateCrawlStats(
  ...
  // Audit sample data
  const sample404Urls: string[] = [];
  pages.forEach(p => {
    if (p.statusCode === 404 || p.isSoft404) {
      if (sample404Urls.length < 10) sample404Urls.push(p.url);
    }
  });
  ...
  const sampleDeadEndArticles: { url: string; title: string }[] = [];
  let articlesWithZeroInlinks = 0;
  let articlesWithOneInlink = 0;
  const isolatedArticlesList: { url: string; title: string; inlinkCount: number }[] = [];
  const articleInlinkMap: { url: string; title: string; inlinkCount: number }[] = [];
  ...
  let totalAnchors = 0;
  let genericCount = 0;
  const externalDomainMap: Record<string, number> = {};
  ...
  articlePages.forEach(p => {
    ...
    const internalOutlinks = (p.outlinks || []).filter((o: Outlink) => !o.isExternal);
