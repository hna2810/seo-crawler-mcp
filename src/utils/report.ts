...
import { CuratedKeyword } from "../keywords/keywordResearcher";
...
function buildSafePageLookup(session: CrawlSession): Map<string, any> {
  const pageLookup = new Map<string, any>();

  const getLookupKeys = (urlStr: string): string[] => {
    const keys = [urlStr];
    try {
  ...
  for (const page of Object.values(session.pages)) {
    if (page.finalUrl) {
      for (const k of getLookupKeys(page.finalUrl)) {
        if (rootKeys.includes(k)) continue;
        if (!pageLookup.has(k)) {
          pageLookup.set(k, page);
        }
      }
...
export function generateInternalLinksCSV(session: CrawlSession): string {
  const headers = [
    "STT",
    "URL bài viết nguồn",
...
export function generateExternalLinksCSV(session: CrawlSession): string {
  const headers = [
    "STT",
    "URL bài viết nguồn",
    "Title bài viết nguồn",
...
export async function generateComprehensiveExcelWorkbook(
  ...
  // Collect internal in-content links from articles only
  ...
  const internalLinksList: ArticleInternalLink[] = [];
  ...
  internalLinksList.forEach((link, idx) => {
    ...
    r.eachCell(c => {
      c.font = { name: "Segoe UI", size: 9 };
      c.border = thinBorder;
      if (idx % 2 === 1) c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFDFBF7" } };
    });
  ...
  const externalLinksList: ArticleExternalLink[] = [];
  ...
  externalLinksList.forEach((link, idx) => {
    ...
    r.eachCell(c => {
      c.font = { name: "Segoe UI", size: 9 };
      c.border = thinBorder;
      if (idx % 2 === 1) c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFF0FDFA" } };
    });
    r.getCell(1).alignment = { vertical: "middle", horizontal: "center" };
    r.getCell(4).alignment = { vertical: "middle", horizontal: "center" };
    r.getCell(5).alignment = { vertical: "middle", horizontal: "center" };
 ...
 */
export function generateKeywordsCSV(keywords: CuratedKeyword[]): string {
  const headers = [
    "STT",
    "Từ Khóa",
    "Lượt Tìm Kiếm / Tháng",
 ...
 */
export async function generateKeywordsExcelWorkbook(
  keywords: CuratedKeyword[],
  summaryInfo?: { totalVolume: number; seedsCount: number }
): Promise<Buffer> {
  const wb = new ExcelJS.Workbook();
